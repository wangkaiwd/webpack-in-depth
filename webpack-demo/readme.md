...
### 提升`webpack`打包速度
*. 使用新的`Node,Npm,Yarn`
*. 在尽可能少的使用`loader/plugins`,每个额外的`loader/plugins`都会增加启动时间
*. `resolve`参数合理配置，减少`resolve.extensions`等配置项，因为它们会增加文件系统的调用次数
*. `DLL`插件优化:  
    a. 如第三方模块等不频繁更改的代码生成单独编译结果，并且每次打包如果文件内容没有更改不重新打包
    b. 引入单独编译的模块(使用dll文件引入)

`DLL`的优化思路： 将一些每次打包不会发生改动的一些第三方模块和项目源代码进行单独打包，然后在最终的打包文件中引入，减少第三方模块的打包时间
...
* 主配置中使用`webpack.DllReferencePlugin`来识别映射文件`manifest.json`中被打包的第三方模块,分别对应到代码中`import`等导入模块的位置，确保代码正确运行

`DLL`操作简化插件： `autodll-webpack-plugin`

* 减少编译结果的体积（小即是快）
* 缓存
* 合理设置`sourceMap`(有时候线上部署使用`sourceMap`可以方便定位线上`bug`)
